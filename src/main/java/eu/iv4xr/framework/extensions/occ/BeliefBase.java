 ...
 * goals, which is essential for our emotion appraisal system.
 * 
 * A "goal" here does not necessarily mean what the agent is programmed to do.
 * Here "goals" represent real life goals, the agent would emotionally cares
 * about.
 * 
 * For example, a software agent simulating a human bus driver might be
 * programmed to drive a bus to reach a destination B without crashing it.
 * "Reaching B" can be taken as a goal. But perhaps "Reaching B on time" and
 * "Picking up many passengers" might also be other goals that are emotionally
 * relevant for a human driver.
 */
public interface BeliefBase {
    ...
    public static class Goals_Status implements Cloneable {
        ...
        public String toString() {
            String r = "";
            for (GoalStatus gs : statuses.values())
                r += "    " + gs.toString() + "\n";
            return r;
        }
